...
function summaryReporter(opts) {
    ...
    let args;
    let favoriteBrowser;
    function log(logger, name, passed, prefix = '  ', postfix = '') {
        const sign = passed ? green('✓') : red('𐄂');
        if (flatten)
            logger.log(`${sign} ${prefix} ${name}${postfix}`);
        else
    ...
    return {
        ...
        reportTestFileResults({ logger, sessionsForTestFile }) {
            ...
            reportBrowserLogs_js_1.reportBrowserLogs(logger, sessionsForTestFile);
        },
        onTestRunFinished({ sessions }) {
            const failedSessions = sessions.filter(s => !s.passed);
            if (failedSessions.length > 0) {
                cachedLogger.log('\n\nErrors Reported in Tests:\n\n');
                reportTestsErrors_js_1.reportTestsErrors(cachedLogger, args.browserNames, favoriteBrowser, failedSessions);
                reportTestFileErrors_js_1.reportTestFileErrors(cachedLogger, args.browserNames, favoriteBrowser, failedSessions, true);
            }
        },
    };
}
