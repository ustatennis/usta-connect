...
import { verifyEmail } from '../../middleware/user.js';

function createDescription() {
  const divElement = document.createElement('div');
  divElement.className = 'verify-initial-auth-description';
  const spanElement1 = document.createElement('span');
  spanElement1.innerText = 'We have sent a password reset code by email to  ';
  const spanElement2 = document.createElement('span');
  spanElement2.innerText = 'your email. ';
  spanElement2.id = 'verifyInitialAuthDescriptionEmail';
...
async function onSendCodeButtonClick() {
  ...
  const sendCodeButton = document.querySelector('#sendButton');
  sendCodeButton.disabled = true;
  sendCodeButton.innerText = '';
  ...
  try {
    ...
    const errorMessage = document.querySelector('.verify-init-auth-err-msg');
...
function createSendCodeButton() {
  ...
  buttonElement.id = 'sendCodeButton';
...
function createErrorMessage() {
  ...
  errorDiv.className = 'verify-init-auth-err-msg';
